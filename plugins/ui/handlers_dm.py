...
# ─────────────────────────────────────────────────────────────────────────────
@Client.on_callback_query(filters.regex(r"^tgl_(local|global|bio_check|anti_mention)_(-?\d+)$"))
async def cb_toggle(client, cb: CallbackQuery):
    await cb.answer()
    try:
        m       = re.match(r"^tgl_(local|global|bio_check|anti_mention)_(-?\d+)$", cb.data)
        key     = m.group(1)
